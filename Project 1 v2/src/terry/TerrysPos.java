...
public class TerrysPos {
	private static int[] itemsOrdered = new int[100]; // pork
	private static int numberOfItemsOrdered = 0; // pork, beef, fish
	...
	private static void showReceipt() {
		showReciptHeader();
		for (int i = 0; i < numberOfItemsOrdered; i++) {
			System.out.printf(MENU_ITEM_FORMAT, numOfItems[itemsOrdered[i]-1], descriptions[itemsOrdered[i]-1],
					subtotal[itemsOrdered[i]]);
			total += subtotal[itemsOrdered[i]];
		}
...
//		
		...
		int num = scan.nextInt(); // get itemNumber
		int index = num - 1;
		if (num != 0 && num != 100) {
			askQuantity(); // get numOfOrders
			itemsOrdered[index] = num; // record what is ordered: pork, beef, etc
